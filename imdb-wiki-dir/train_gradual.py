'''
codes for Bayesian deep learning
'''
import time
import argparse
import logging
...
from loss import *
from datasets_gradual import IMDBWIKI
from utils import *
...
parser.add_argument('--loss', type=str, default='l1nll', choices=['l1nll', 'mse', 'l1', 'focal_l1', 'focal_mse', 'huber'], help='training loss type')
...
def main():
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            # cp best model to new folder
            best_model = args.resume.replace('pth.tar', 'best.pth.tar')
            shutil.copyfile(best_model, f"{args.store_root}/{args.store_name}/ckpt.best.pth.tar")
    ...
    print(f'total trainable params number: {sum(p.numel() for p in model.parameters() if p.requires_grad)}')
    ...
    if args.fds and epoch >= args.start_update:
        ...
        with torch.no_grad():
            for (inputs, targets, _) in tqdm(train_loader):
                ...
                encodings.extend(feature.data.squeeze(-1).cpu().numpy())
                labels.extend(targets.data.squeeze(-1).cpu().numpy())
...
def validate(val_loader, model, train_labels=None, prefix='Val', umetric=True):
    ...
    with torch.no_grad():
        ...
        for idx, (inputs, targets, _) in enumerate(val_loader):
            ...
            outputs = model(inputs)
            # get mu from outputs
            if 'nll' in args.loss or 'sigma' in args.loss:
