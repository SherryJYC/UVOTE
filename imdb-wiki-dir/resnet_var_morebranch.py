...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, fds, bucket_num, bucket_start, start_update, start_smooth,
                 kernel, ks, sigma, momentum, dropout=None, return_features=False, num_branch=2):
        self.inplanes = 64
        ...
        self.mu_linears = nn.ModuleList([nn.Linear(512 * block.expansion, 1) for _ in range(num_branch)])
        self.logvar_linears = nn.ModuleList([nn.Linear(512 * block.expansion, 1) for _ in range(num_branch)])
    ...
    def forward(self, x, targets=None, epoch=None):
        ...
        for i in range(self.num_branch):
            mu = self.mu_linears[i](encoding_s)
            logvar = self.logvar_linears[i](encoding_s)
            out.extend([(mu, logvar)])
