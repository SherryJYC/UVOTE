...
def combine_dataset(path='meta'):
    ...
    data_imdb['path'] = data_imdb['path'].apply(lambda x: "imdb_crop/{}".format(x))
    data_wiki['path'] = data_wiki['path'].apply(lambda x: "wiki_crop/{}".format(x))
...
def create(db):
    args = get_args()
    mat_path = os.path.join(args.data_path, "{}_crop".format(db), "{}.mat".format(db))
    ...
    output_path = os.path.join(output_dir, "{}.csv".format(db))
