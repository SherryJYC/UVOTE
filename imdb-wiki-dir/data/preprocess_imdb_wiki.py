...
def make_balanced_testset(db="imdb_wiki", max_size=150, seed=666, verbose=True, vis=True, save=False):
    ...
    if verbose:
        print("Val: {}\nTest: {}".format(len(val_set), len(test_set)))
    ...
    if verbose:
        print(df)
    if save:
        df.to_csv(str(join(BASE_PATH, "{}.csv".format(db))), index=False)
    if vis:
        ...
        ax[0].set_title("[{}] train: {}".format(db.upper(), df_train.shape[0]))
        ax[1].hist(df[df['split'] == 'val']['age'], range(max(df['age'])))
        ax[1].set_title("[{}] val: {}".format(db.upper(), df[df['split'] == 'val'].shape[0]))
        ax[2].hist(df[df['split'] == 'test']['age'], range(max(df['age'])))
        ax[2].set_title("[{}] test: {}".format(db.upper(), df[df['split'] == 'test'].shape[0]))
        ax[0].set_xlim([0, 120])
        plt.tight_layout()
        plt.show()
