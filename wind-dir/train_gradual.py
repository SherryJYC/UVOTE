...
from dataset_fit import Wind
from utils import *
from loss import *
from resnet_var_morebranch import resnet18
...
parser.add_argument('--bucket_num', type=int, default=185, help='maximum bucket considered for FDS')
...
parser.add_argument('--dataset', type=str, default='wind', help='dataset name')
...
parser.add_argument('--loss', type=str, default='l1', choices=['l1nll', 'mse', 'l1', 'focal_l1', 'focal_mse', 'huber'], help='training loss type')
...
def main():
    ...
    train_labels = df_train['wind_speed']

    train_dataset = Wind(data_dir=args.data_dir, df=df_train, img_size=args.img_size, split='train',
                          reweight=args.reweight, lds=args.lds, lds_kernel=args.lds_kernel, lds_ks=args.lds_ks, lds_sigma=args.lds_sigma)
    val_dataset = Wind(data_dir=args.data_dir, df=df_val, img_size=args.img_size, split='val')
    test_dataset = Wind(data_dir=args.data_dir, df=df_test, img_size=args.img_size, split='test')
...
def train(train_loader, model, optimizer, epoch):
    ...
    for idx, (inputs, targets, weights) in enumerate(train_loader):
        ...
        for i in range(len(weights)):
            ...
            if loss is None:
                if args.dynamic_loss:
                    loss = alpha * globals()[f"weighted_{args.loss}_loss"](mu, log_var, targets, weights[i])
                else:
