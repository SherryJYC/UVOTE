...
#from tasks_morebranch import STSBTask
from tasks_morebranch_woKDE import STSBTask
...
def process_split(split, indexers):
    ...
    elif len(split) == 5:   # weight_inv, weight_sqrt
        weights_inv = [NumericField(w) for w in split[2]]
        weights_sqrt = [NumericField(w) for w in split[3]]
        instances = [Instance({"input1": input1, "input2": input2, "label": label, 
                    "weight_inv": weight_inv, "weight_sqrt": weight_sqrt, }) for \
                     (input1, input2, label, weight_inv, weight_sqrt) in zip(inputs1, inputs2, labels, weights_inv, weights_sqrt)]
    else:
