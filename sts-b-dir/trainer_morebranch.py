...
def build_trainer(args, model, iterator):
    '''Build a trainer'''
    parameters = [n for n, p in model.named_parameters() if 'logvar' not in n and p.requires_grad]
    parameters_logvar = [n for n, p in model.named_parameters() if 'logvar' in n and p.requires_grad]

    opt_params = Params({'type': args.optimizer, "parameter_groups": [
        [parameters, {"lr": args.lr}], [parameters_logvar, {"lr": args.lr * 0.1 }]
    ], 'weight_decay': 1e-5})
    ...
    def train(self, tasks, validation_interval, train_params, optimizer_params, resume=False):
        ...
        while not should_stop:
            ...
            if time.time() - task_info['last_log'] > self._log_interval:
                ...
                for i in range(self._model.args.num_branch):
                    task_metrics[f"%s_logvar_{i}" % task.name] = logvars[i] / n_batches_since_val
            ...
            # check should_stop using max_epoch
            if real_epoch >= MAX_EPOCH:
                should_stop = True
                logging.info('Stopped training after reaching max_epoch %d', MAX_EPOCH)
