...
from preprocess_morebranch import build_tasks
from models_morebranch import build_model
from trainer_morebranch import build_trainer
from evaluate_morebranch import evaluate
...
def main(arguments):
    ...
    parser.add_argument('--max_vals', help='maximum number of validation checks', type=int, default=200)
    ...
    parser.add_argument('--reweight', type=str, default='none', choices=['none', 'sqrt_inv', 'inverse', 'two'],
                        help='cost-sensitive reweighting scheme')

    # for more branch settting
    parser.add_argument('--num_branch', help='num of branch of model', type=int, default=1)
    parser.add_argument('--max_epoch', help='num of max epoch', type=int, default=200)
    parser.add_argument('--dynamic_loss', action='store_true', default=False,
                        help='whether to combine loss (like two) dynamically')
    ...
    if args.dynamic_loss:
        args.store_name += f'_dyL_{args.max_epoch}_numb_{args.num_branch}'

    args.store_name += f'_seed_{args.random_seed}_valint_{args.val_interval}_patience_{args.patience}' \
                       f'_{args.optimizer}_{args.loss}_{args.lr}_{args.batch_size}'
    ...
    model_state = torch.load(model_path, map_location=device_mapping(args.cuda))
    model = resume_checkpoint(model, model_state, morebranch=True)
